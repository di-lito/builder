...
class Su_Shortcode_heading extends Su_Shortcodes {
  ...
  public static function heading( $atts = null, $content = null ) {
    ...
    if ($atts['line_height']) {
        $line_height = 'line-height: ' . intVal($atts['line_height']) . 'px;';
    } else {
        $line_height = ($atts['size']) ? 'line-height: ' . intVal($atts['size']) . 'px;' : '';
    }
    ...
    if ($atts['size'] or $atts['color'] or $line_height) {
        $css[] = '#'.$id.'.su-heading .su-heading-inner {'.$size.$color.$line_height.'}';
    }
