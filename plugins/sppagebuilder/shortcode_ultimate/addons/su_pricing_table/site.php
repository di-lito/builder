...
class SppagebuilderAddonSu_pricing_table extends SppagebuilderAddons{
	...
	public function render(){
		...
		foreach($this->addon->settings->pricing_plan as $key => $plan) {
			...
			$old_price            = (isset($plan->old_price) && $plan->old_price) ? $plan->old_price : '';
			$after                = (isset($plan->after) && $plan->after) ? $plan->after : '';
			$btn_target           = (isset($plan->btn_target) && $plan->btn_target) ? $plan->btn_target : '';
			$badge                = (isset($plan->badge) && $plan->badge) ? $plan->badge : '';
